...
class DERManager:
    ...
    def __init__(self, net: pp.pandapowerNet):
        ...
        Parameters:
            net (pp.pandapowerNet): Pandapower network where DER elements will be created and managed. Initializes internal lists and mappings for DER specifications, pandapower element indices, and battery state-of-charge.
        """
        self.net = net
    ...
    def set_solar_output(self, name: str, irradiance_w_m2: float) -> None:
        ...
        Calculates the PV output from panel area, irradiance (W/m²), and module efficiency, clamps it to the DER's capacity, and writes the resulting MW value to the associated pandapower sgen element's p_mw.
        
        Parameters:
            name (str): Name of the solar PV DER.
            irradiance_w_m2 (float): Incident solar irradiance in watts per square meter (W/m²).
        
        Raises:
            ValueError: If the named DER is not of type SOLAR_PV.
        """
    ...
    def set_wind_output(self, name: str, wind_speed_m_s: float) -> None:
        ...
        Computes a capacity factor using a cubic-like curve between 3 m/s (cut-in) and 15 m/s (rated), clamps it to 0–1, multiplies by the DER's capacity, and updates the associated sgen element's active power.
        
        Parameters:
        	name (str): Name of the wind DER.
        	wind_speed_m_s (float): Wind speed in meters per second.
        
        Raises:
        	ValueError: If the named DER is not of type WIND.
        """
        spec = self._get_spec(name)
    ...
    def set_battery_power(self, name: str, power_mw: float) -> None:
        ...
        Parameters:
            name (str): Identifier of the battery DER.
            power_mw (float): Desired active power in megawatts; positive = discharge, negative = charge. The value is clamped to the battery's configured charge/discharge limits.
        
        Raises:
            ValueError: If the named DER is not a battery (BESS).
        """
        spec = self._get_spec(name)
    ...
    def set_ev_charging_load(self, name: str, utilization: float, smart_override: bool = False) -> None:
        ...
        Parameters:
            name (str): Identifier of the EV charging station.
            utilization (float): Fraction between 0 and 1 representing fraction of chargers in use (0 = no chargers active, 1 = all chargers active).
            smart_override (bool): When True and the station has smart charging enabled, reduce the target load by up to 50% for grid balancing.
        
        Behavior:
            Updates the pandapower load for the station: sets active power (p_mw) to the computed target and reactive power (q_mvar) to 10% of that value.
        
        Raises:
            ValueError: If the named DER is not an EV charging station.
        """
        spec = self._get_spec(name)
